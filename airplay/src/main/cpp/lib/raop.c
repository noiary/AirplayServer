...
conn_init(void *opaque, unsigned char *local, int locallen, unsigned char *remote, int remotelen) {
    ...
    if (locallen == 4) {
        ...
        logger_log(conn->raop->logger, LOGGER_INFO,
                   ...
                   local[8], local[9], local[10], local[11], local[12], local[13], local[14],
                   local[15]);
    }
    if (remotelen == 4) {
        logger_log(conn->raop->logger, LOGGER_INFO,
        ...
        logger_log(conn->raop->logger, LOGGER_INFO,
                   ...
                   remote[0], remote[1], remote[2], remote[3], remote[4], remote[5], remote[6],
                   remote[7],
                   remote[8], remote[9], remote[10], remote[11], remote[12], remote[13], remote[14],
                   remote[15]);
    }
